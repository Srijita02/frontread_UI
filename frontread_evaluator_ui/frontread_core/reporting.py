...
def _model(result: dict[str, Any], role: str) -> dict[str, Any]:
    ...
    if role in models:
        return models.get(role) or {}
    if role == "text_generator":
        return result.get("generator", {}) or {}
    if role in {"text_evaluator", "question_evaluator"}:
        return result.get("evaluator", {}) or {}
...
def summary_rows(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for result in results:
        ...
        prompting = result.get("prompting", {})
        row = {
            ...
            "text_approved": approval.get("text_approved"),
            "questions_approved": approval.get("questions_approved"),
            "overall_approved": approval.get("overall_approved"),
...
def text_metric_rows(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for result in results:
        ...
        row.update(flatten_ai_scores(result.get("text_ai_eval", {}), "text_ai"))
        rows.append(row)
    return rows
...
def question_metric_rows(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for result in results:
        ...
        row.update(flatten_ai_scores(result.get("question_ai_eval", {}), "question_ai"))
        rows.append(row)
    return rows
...
def question_rows(results: list[dict[str, Any]]) -> list[dict[str, Any]]:
    rows = []
    for result in results:
        for q in result.get("questions", {}).get("questions", []):
            rows.append(
                {
                    "run_item_id": result.get("run_item_id"),
                    "topic": result.get("topic"),
                    "grade": result.get("grade_label"),
                    "title": result.get("text", {}).get("title"),
                    "number": q.get("number"),
...
def save_run(results: list[dict[str, Any]], run_config: dict[str, Any] | None = None) -> dict[str, Path]:
    ...
    (run_dir / "results.json").write_text(json.dumps(clean, ensure_ascii=False, indent=2), encoding="utf-8")
